...
module.exports = {
  ...
  getLongPollService(config) {
    const { configServerUrl, appId, clusterName, notifications } = config;
    assert(configServerUrl, 'configServerUrl is required');
    assert(clusterName, 'namespaceName is required');
    assert(appId, 'appId is required');
    assert(notifications, 'clusterName is required');
    // console.log(`${configServerUrl}/notifications/v2/?appId=${appId}&cluster=${clusterName}&notifications=${JSON.stringify(notifications)}`);
    
    return `${configServerUrl}/notifications/v2/?appId=${appId}&cluster=${clusterName}&notifications=${JSON.stringify(notifications)}`;
  }
};
